...
if __name__ == '__main__':
    for i in range(5):
        ...
        for j in range(4):
            ...
            train_file = f"{config.MY_DATA_DIR}/deephage_original_contig/{group}/fold_{fold}/train/data.csv"
            valid_file = f"{config.MY_DATA_DIR}/deephage_original_contig/{group}/fold_{fold}/test/data.csv"
            log.info(f"Fold: {fold}, group: {group}")
            log.info(f"train_file: {train_file}")
            log.info(f"valid_file: {valid_file}")

            output_dir = os.path.join(config.DNA_BERT_2_OUTPUT_DIR_DEEPHAGE_ORIGINAL, f"{group}/fold_{fold}")
