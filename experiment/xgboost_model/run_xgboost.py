...
if __name__ == '__main__':
    for i in range(4):
        if i == 0:
            ...
            x_train_file = os.path.join(config.MY_DATA_DIR, "dna2vec/100_400/dna2vec_100_400_train_vector.npy")
            y_train_file =  os.path.join(config.MY_DATA_DIR, "dna2vec/100_400/y_train.npy")
            x_val_file =  os.path.join(config.MY_DATA_DIR, "dna2vec/100_400/dna2vec_100_400_val_vector.npy")
            y_val_file = os.path.join(config.MY_DATA_DIR, "dna2vec/100_400/y_val.npy")
            run_experiment(range_length, x_train_file, y_train_file, x_val_file, y_val_file)
            # run_fine_tuning()
        elif i == 1:
            range_length = '400_800'
            utils.start_experiment(f"xgboost_{range_length}", time.time())
            log.info(range_length)
            x_train_file = os.path.join(config.MY_DATA_DIR, f"dna2vec/{range_length}/dna2vec_{range_length}_train_vector.npy")
            y_train_file = os.path.join(config.MY_DATA_DIR, f"dna2vec/{range_length}/y_train.npy")
            x_val_file = os.path.join(config.MY_DATA_DIR, f"dna2vec/{range_length}/dna2vec_{range_length}_val_vector.npy")
            y_val_file = os.path.join(config.MY_DATA_DIR, f"dna2vec/{range_length}/y_val.npy")
            run_experiment(range_length, x_train_file, y_train_file, x_val_file, y_val_file)
            # run_fine_tuning()
        elif i == 2:
            ...
            utils.start_experiment(f"xgboost_{range_length}", time.time())
            log.info(range_length)
            x_train_file = os.path.join(config.MY_DATA_DIR,
                                        f"dna2vec/{range_length}/dna2vec_{range_length}_train_vector.npy")
            y_train_file = os.path.join(config.MY_DATA_DIR, f"dna2vec/{range_length}/y_train.npy")
            x_val_file = os.path.join(config.MY_DATA_DIR,
                                      f"dna2vec/{range_length}/dna2vec_{range_length}_val_vector.npy")
            y_val_file = os.path.join(config.MY_DATA_DIR, f"dna2vec/{range_length}/y_val.npy")
            run_experiment(range_length, x_train_file, y_train_file, x_val_file, y_val_file)
            # run_fine_tuning()
        else:
