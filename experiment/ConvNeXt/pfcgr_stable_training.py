...
def init_model(CONFIG):
    ...
    # Create new convolution layer with 5 input channels
    new_conv = nn.Conv2d(
        in_channels=5,  # Changed from 1 to 5 for your 5-channel images
    ...
    # Initialize weights for 5-channel input
    if hasattr(original_conv, 'weight'):
        # Option 1: Replicate RGB weights across all 5 channels
        # This repeats the pretrained RGB pattern
        original_weight = original_conv.weight.data  # Shape: [out_channels, 3, kernel_h, kernel_w]
        new_weight = torch.zeros(original_conv.out_channels, 5,
                                 original_conv.kernel_size[0], original_conv.kernel_size[1])

        # Copy RGB weights to first 3 channels
        new_weight[:, :3, :, :] = original_weight

        # For channels 4 and 5, you can either:
        # Option A: Use the mean of RGB channels
        new_weight[:, 3:, :, :] = original_weight.mean(dim=1, keepdim=True).repeat(1, 2, 1, 1)

        # Option B: Initialize randomly (uncomment if preferred)
        # nn.init.kaiming_normal_(new_weight[:, 3:, :, :], mode='fan_out', nonlinearity='relu')

        # Option C: Duplicate one of the RGB channels (e.g., green channel)
        # new_weight[:, 3, :, :] = original_weight[:, 1, :, :]  # Copy green
        # new_weight[:, 4, :, :] = original_weight[:, 1, :, :]  # Copy green
