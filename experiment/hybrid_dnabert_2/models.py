...
def load_dnabert2_for_classification():
    """
    Option 1: Sử dụng trực tiếp AutoModelForSequenceClassification (Đơn giản nhất)
    Tương tự như code hiện tại của bạn
    """
...
class DNABERT2ForPhageClassification(PreTrainedModel):
    """
    Option 2: Custom model với Frozen Encoder
    Kế thừa PreTrainedModel để tương thích với Hugging Face Trainer
    """
    ...
    def __init__(self, config):
        ...
        # Load pre-trained DNABERT-2 backbone
        ...
        # Freeze DNABERT-2 parameters (feature extractor mode)
        for param in self.dnabert.parameters():
            param.requires_grad = False

        print("DNABERT-2 parameters frozen - using as feature extractor")
        ...
        self.classifier = nn.Sequential(
            nn.Linear(768, 512),  # DNABERT-2 hidden size is 768
            nn.ReLU(),
    ...
    def forward(self, input_ids=None, attention_mask=None, labels=None, **kwargs):
        ...
        if attention_mask is not None:
            # Mask padding tokens
            attention_mask = attention_mask.unsqueeze(-1).expand(sequence_output.size()).float()
            sum_embeddings = torch.sum(sequence_output * attention_mask, 1)
            sum_mask = attention_mask.sum(1)
            sum_mask = torch.clamp(sum_mask, min=1e-9)
            pooled_output = sum_embeddings / sum_mask
        else:
            pooled_output = torch.mean(sequence_output, dim=1)  # Simple mean pooling
