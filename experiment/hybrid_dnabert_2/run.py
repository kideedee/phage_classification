...
from models import (
        DNABERT2ForPhageClassification,
        DNABERT2_CNN_HybridForPhageClassification,
        create_custom_config,
        load_dnabert2_for_classification
    )
...
def load_model(model_type="default", device="cuda"):
    ...
    if model_type == "default":
        # Option 1: Sử dụng AutoModelForSequenceClassification như code gốc
        log.info("Loading default DNABERT-2 for sequence classification...")
        model = load_dnabert2_for_classification()

    elif model_type == "frozen":
        # Option 2: Custom model với frozen encoder
        log.info("Loading DNABERT-2 with frozen encoder...")
        config = create_custom_config()
        model = DNABERT2ForPhageClassification(config)
