...
def run():
    for i in range(4):
        ...
        for j in range(5):
            fold = j + 1
            if fold == 1:
                data_dir = os.path.join(config.PHATYP_FILTER_DNA_BERT_S_TOKENIZER_DATA_DIR, f"{group}/fold_{fold}")
            elif fold == 2:
                data_dir = os.path.join(config.PHATYP_FILTER_DNA_BERT_S_TOKENIZER_DATA_DIR, f"{group}/fold_{fold}")
            elif fold == 3:
                data_dir = os.path.join(config.PHATYP_FILTER_DNA_BERT_S_TOKENIZER_DATA_DIR, f"{group}/fold_{fold}")
            ...
            utils.start_experiment(f"finetune_dna_bert_S_group_{group}_fold_{fold}", time.time())
